...
async function init(options = {}) {
  ...
  try {
    ...
    // Handle CLAUDE.md
    await updateClaudeMd(claudeMdPath, options);
...
${forgeSection}`;
      ...
      console.log(chalk.gray('  ✓ Created CLAUDE.md with forge instructions'));
    } else {
      // Check if forge section already exists
      const content = await fs.readFile(claudeMdPath, 'utf8');
      
      if (content.includes('Claude Forge')) {
        console.log(chalk.yellow('  ⚠️  CLAUDE.md already contains Claude Forge section'));
      } else {
        // Append forge section
        const updatedContent = content.trimEnd() + '\n' + forgeSection;
        await fs.writeFile(claudeMdPath, updatedContent);
        console.log(chalk.gray('  ✓ Added Claude Forge section to CLAUDE.md'));
      }
    }
  } catch (error) {
